...
def welcome_redrawAll(app):
    ...
    for i in range(9):
        drawLine(app.width/9 + i * 100, 0, app.width/9 + i * 100, 
                    app.height, lineWidth =1, fill='white')
    ...
    if app.textShown:
        drawLabel("Press space to begin!", app.width/2 , app.height-200, 
                    size = app.textSize, font='monospace', fill='white', 
                    bold=True)
...
def game_redrawAll(app):
    # if the AI is about to make a random move, draw confirmation message on the scren
    ...
    # otherwise, draw the game board to continue
        ...
        drawLabel("Mine Smarter!", app.width/2, 40, size = 60, font='monospace', 
                fill='white', bold = True)
        if app.message != None:
            drawLabel(app.message, app.width/2, 100, size = app.messageSize, font='monospace', 
                fill='white', bold = True)
        app.minesweeper.drawGrid()

def game_drawAIRandomConfirmation(app):
    """
    This is part of the functionality that tells the user if the AI is going to make a random move
    Draws a confirmation screen
    """
    # draw blue background rectangle
    drawRect(0, 0, app.width, app.height, fill='blue')
    # draw confirmation text
    drawLabel("ALERT", app.width//2, 100, fill='red', size=40, font='monospace', bold = True)
    drawLabel("Heads up! AI is about to make a random move.", app.width//2, app.height//2 - 80, 
              fill='white', size=25, font='monospace', bold = True)
    drawLabel("Press 'y' to continue, 'n' to make your own move", app.width//2, app.height//2 + 80, 
            fill='white', size=25, font='monospace', bold = True)
...
def game_onMousePress(app, mouseX, mouseY):
    ...
    if not app.gameOver:
        ...
        if (app.minesweeper.flagBoxLeft <= mouseX <= app.minesweeper.flagBoxLeft + app.minesweeper.flagBoxWidth
            ...
            # we are clicking on a flag, so set the cursor in the game to be a flag cursor
        ...
        if (app.minesweeper.AIBoxLeft <= mouseX <= app.minesweeper.AIBoxLeft + app.minesweeper.AIBoxWidth
            ...
            if move == None:
                # if the move is none, that means the AI could not make a safe move (doesn't know enough information)
                # the AI has to make a random move, but the user should be alerted that the AI is going to make a random move.
...
def game_onStep(app):
    ...
    if app.minesweeper.gameOver:
        # if the game is over, stop all gameplay.
...
def game_makeAIRandomMove(app):
    # make random move only returns a valid random move (that is, not a random move where the AI knows a mine is there.)
...
def game_onKeyPress(app, key):
    ...
    if key == 'y' and app.AIGoingRandomMove:
        ...
        if move == None:
            # if the move is still none, that means that the game has been won (alll the cells that are available are the AI's known mines)
            # stop gameplay
